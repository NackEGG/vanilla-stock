...
BEGIN	
	V_QUERY:= 'SELECT TB.TITLE, TB.STOCK_CODE, TB.MEMBER_NO, TB.REGDATE, TB.COMPANY_NAME, TB.COUNT_HIT, TB.COUNT_YES, TB.COUNT_NO
		FROM (SELECT A.TITLE, A.STOCK_CODE, A.MEMBER_NO, A.REGDATE,
				...
				(SELECT COUNT(*) FROM PICK P WHERE P.ARTICLE_NO = A.NO AND OPINION = '||'''Y'''||') COUNT_YES,
 				(SELECT COUNT(*) FROM PICK P WHERE P.ARTICLE_NO = A.NO AND OPINION = '||'''N'''||') COUNT_NO
		FROM ARTICLE A ';
	...
	IF PI_SORT_TYPE = 'latest' THEN
		...
		ELSIF PI_SORT_TYPE = 'popularity' THEN
			V_QUERY:= V_QUERY || ' ORDER BY (COUNT_HIT + COUNT_YES + COUNT_NO) DESC) TB ';
	END IF;
