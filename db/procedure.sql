...
-- 해당 댓글 좋아요 등록
CREATE OR REPLACE PROCEDURE USP_INSERT_LIKES(
	PI_COMMENT_NO IN LIKES.COMMENT_NO%TYPE,
	PI_MEMBER_NO IN LIKES.MEMBER_NO%TYPE
)
AS
BEGIN
	INSERT INTO LIKES
	VALUES(LIKES_SEQ.NEXTVAL, PI_COMMENT_NO, PI_MEMBER_NO, SYSDATE);
END;
/

-- 해당 댓글 좋아요 취소
CREATE OR REPLACE PROCEDURE USP_DELECT_LIKES(
	PI_COMMENT_NO IN LIKES.COMMENT_NO%TYPE,
	PI_MEMBER_NO IN LIKES.MEMBER_NO%TYPE
)
AS
BEGIN
	DELETE FROM LIKES
	WHERE COMMENT_NO = PI_COMMENT_NO AND MEMBER_NO = PI_MEMBER_NO;
END; 
/

-- 해당 댓글 좋아요 수 집계
CREATE OR REPLACE PROCEDURE USP_GET_COUNT_LIKES(
	PI_COMMENT_NO IN LIKES.COMMENT_NO%TYPE,
	PO_COUNT_LIKES OUT NUMBER
)
AS
BEGIN
	SELECT COUNT(*)
	INTO PO_COUNT_LIKES
	FROM LIKES
	WHERE COMMENT_NO = PI_COMMENT_NO;
END;
/